"""
04_visualize_a2c.py — A2C (TD) visualization
Highlights the flat advantage failure on DoorKey environments
"""
import sys, os
...
ALGO  = 'a2c_td'
LABEL = 'A2C (TD Bootstrapped)'
COLOR = 'tomato'
OUT   = os.path.join(PLOTS_DIR, 'a2c_td')
...
# D) Empty vs DoorKey degradation (A2C specific insight)
print("[D] Degradation across env complexity...")
fig, axes = plt.subplots(1, 2, figsize=(14, 5))
fig.suptitle(f'{LABEL} — Performance Degradation with Environment Complexity',
...
for col, (group, env_group) in enumerate([('Empty', ENVS_EMPTY), ('DoorKey', ENVS_DOORKEY)]):
    ...
    for decay in DECAYS:
        ...
        ax.plot(sizes, srs, color=DECAY_COLORS[decay], ls=DECAY_STYLES[decay],
                lw=2, marker='o', ms=8, label=DECAY_LABELS[decay])
    ax.set_title(f'{group} Environments'); ax.set_xlabel('Environment Size')
    ax.set_ylabel('Success Rate (last 500 eps)'); ax.legend(fontsize=8)
    ax.grid(True, alpha=0.3); ax.set_ylim(-0.05, 1.05)
plt.tight_layout()
savefig(fig, f'{OUT}/{ALGO}__degradation.png')

print(f"\nDone! All A2C plots saved to {OUT}/")
